...
def find_shot_hole(gray, tx, ty, r, search_radius):
    ...
    - The inside/outside zones used to split at r*0.95 and r*1.05, leaving a
      ...
      The zones now split at the fitted radius r itself (no gap); the existing
      intensity thresholds (>170 / <140) already keep the sharp printed edge
      from being misread as a hole, so no buffer band is needed.
...
def analyze_orion_target(image_path, output_path="scored_output_warped.jpg"):
    ...
    for idx, (tx, ty, r) in enumerate(targets_to_score, start=1):
        ...
        # search radius spans the real usable scoring area around this
